...
//                pixel_costs = costs + (c+r*cover.cols)*3];
//                System.arraycopy(costs, (c+r*cover.cols)*3, pixel_costs, 0, pixel_costs.length);
                pixel_costs[0] = 0;
                pixel_costs[1] = 0;
                pixel_costs[2] = 0;
                calc_costs(r+3, c+3, cover_padded, pixel_costs);
                costs[(c+r*cover.cols)] = pixel_costs[0];
                costs[(c+r*cover.cols)+1] = pixel_costs[1];
                costs[(c+r*cover.cols)+2] = pixel_costs[2];
            }
        }
    }
