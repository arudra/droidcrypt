...
int main(){
    ...
    HUGO_like(NULL, 200, 200, "1234567890", num_bits_used);
...
//        std::cout <<"Here we get all the information: password " << tmp << std::endl;
        
//        LOGI("Here we get all the information: password =%s (%d)", tmp, len);
        delete tmp;
//        tmp = NULL;
        long tmpAddr = (long)tmp;
        std::cout <<"Here we get all the information: password " << std::hex << (tmpAddr) << "  " << tmpAddr + len << std::endl;
        ...
        std::cout << "Addr for config: " << std::hex << (long) config << std::endl;
        // Load cover
        LOGI("Loading the cover image");
        mat2D<int> *cover = NULL;
        cover = Mat2dFromImage(img, width, height);
        ...
        if (!isSame) {
            ...
            std::cout <<"Error: Embedded Message and Extracted message are not the same!" << std::endl;
            num_bits_used[0] = 0;
            num_bits_used[1] = 0;
        }
        else {
...
//            LOGI("Successfully extracted_message is %s", tmp);
            std::cout <<"Successfully extracted_message is " << std::endl;
